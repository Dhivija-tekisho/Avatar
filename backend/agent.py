...
class Assistant(Agent):
    ...
    async def search_client_in_database(self, name: str, company: str) -> str:
        ...
        try:
            ...
            # Search by company first, then by name
            ...
            if client_doc:
                # Extract data from Supabase record
                ...
                record_research_about_company = client_doc.get('industry', '')
                record_company_summary = client_doc.get('description', '')
                ...
                logger.info(f"Found client in Supabase: {record_name} from {record_company_name}")
                return response
            else:
    ...
    async def store_chat_history(self, chat_messages: list) -> dict:
        ...
        try:
            ...
            logger.info(f"Successfully stored chat history for {client_name} from {company_name}")
